...
class Scorer:    
    ...
    def compute_scores_with_vector_space_model(self, query, method):
        ...
        for doc_id in self.get_list_of_documents(query):
            scores[doc_id] = self.get_vector_space_model_score(query, query_tfs, doc_id, method, method)
        return scores
    ...
    def get_vector_space_model_score(self, query, query_tfs, document_id, document_method, query_method):
        ...
        """
        query_vector = [0 for _ in range(len(self.index))]
        document_vector = [0 for _ in range(len(self.index))]

        for term, tf_doc in self.index.items():
            query_tf = 1 + np.log2(query_tfs.get(term, 0)) if query_method[0] == 'l' else query_tfs.get(term, 0)
            query_df = self.get_idf(term) if query_method[1] == 't' else 1
            query_vector.append(query_df * query_tf)
            doc_tf = 1 + np.log2(tf_doc.get(document_id, 0)) if document_method[0] == 'l' else tf_doc.get(document_id, 0)
            doc_df = self.get_idf(term) if document_method[1] == 't' else 1
            document_vector.append(doc_tf * doc_df)

        if query_method[2] == 'c':
            query_vector = np.array(query_vector) / np.linalg.norm(query_vector)
        if document_method[2] == 'c':
            doc_vector = np.array(document_vector) / np.linalg.norm(document_vector)
    ...
    def get_okapi_bm25_score(self, query, document_id, average_document_field_length, document_lengths):
        ...
        for term, tf in query_tfs.items():
            ...
            if document_id in self.index.get(term, {}):
                tf_d = len(self.index[term][document_id])
            else:
                tf_d = 0
            dl = document_lengths[document_id]
            score += idf * ((tf_d * (k1 + 1)) / (tf_d + k1 * (1 - b + b * (dl / average_document_field_length))))
        return score
    ...
    def compute_scores_with_unigram_model(
        ...
        float
            A dictionary of the document IDs and their scores.
        """
        return {doc: self.compute_score_with_unigram_model(query, doc, smoothing_method, document_lengths, alpha, lamda)
                for doc in document_lengths.keys()}
    ...
    def compute_score_with_unigram_model(
        ...
        for term in query.split():
            if smoothing_method == 'bayes':
                score += math.log(
                    (self.index[term][document_id] + alpha) / (document_lengths[document_id] + alpha * len(self.index)))
            elif smoothing_method == 'naive':
                score += math.log(
                    (self.index[term][document_id] + 1) / (document_lengths[document_id] + len(self.index)))
            else:
                score += (lamda * (self.index[term][document_id] / document_lengths[document_id]) + (1 - lamda) * (collection[term] / collection_size))
        return score
